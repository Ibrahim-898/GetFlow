

// // import React, { useEffect, useState, useCallback } from 'react';
// // import { LineChart, Line, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer } from 'recharts';
// // import './Dashboard.css';
// // import { apiKeyAPI } from '../../services/apiKeyAPI';
// // import { analyticsAPI } from '../../services/analytics';

// // const Dashboard = () => {
// //   const [logs, setLogs] = useState([]);
// //   const [apiKeys, setApiKeys] = useState([]);        // Fixed destructuring
// //   const [requestsPerMinute, setRequestsPerMinute] = useState([]);
// //   const [loading, setLoading] = useState(true);
// //   const [error, setError] = useState('');

// //   // Process logs for chart
// //   const processRequestsPerMinute = useCallback((logsData) => {
// //     const counts = {};
// //     logsData.forEach((log) => {
// //       if (!log.createdAt) return;
// //       const d = new Date(log.createdAt);
// //       const key = `${d.getHours().toString().padStart(2, '0')}:${d.getMinutes()
// //         .toString().padStart(2, '0')}`;
// //       counts[key] = (counts[key] || 0) + 1;
// //     });

// //     const chartData = Object.keys(counts)
// //       .map((time) => ({ time, requests: counts[time] }))
// //       .sort((a, b) => a.time.localeCompare(b.time));

// //     setRequestsPerMinute(chartData);
// //   }, []);

// //   // Fetch API Keys
// //   const fetchApiKeys = useCallback(async () => {
// //     try {
// //       const response = await apiKeyAPI.getAll();
// //       const userKeys = response.data.data;
// //       setApiKeys(userKeys);
// //     } catch (err) {
// //       console.error('Failed to fetch API keys:', err);
// //     }
// //   }, []);

// //   // Fetch Logs - Oldest first, Newest last
// //   const fetchLogs = useCallback(async () => {
// //     try {
// //       setLoading(true);
// //       setError('');

// //       const res = await analyticsAPI.getAnalytics();
// //       let logsData = res.data.data || res.data || [];

// //       console.log("Raw logs received:", logsData);

// //       // Sort: Oldest → Newest (Newest date and time at the bottom)
// //       const sortedLogs = [...logsData].sort((a, b) => 
// //         new Date(a.createdAt) - new Date(b.createdAt)
// //       );

// //       setLogs(sortedLogs);
// //       processRequestsPerMinute(sortedLogs);
// //     } catch (err) {
// //       console.error('Failed to fetch logs:', err);
// //       setError('Failed to load analytics logs');
// //     } finally {
// //       setLoading(false);
// //     }
// //   }, [processRequestsPerMinute]);

// //   // Load data when component mounts
// //   useEffect(() => {
// //     fetchLogs();
// //     fetchApiKeys();
// //   }, [fetchLogs, fetchApiKeys]);

// //   return (
// //     <div className="dashboard-page">
// //       <div className="dashboard-container">
// //         <div className="dashboard-header">
// //           <h1>API Analytics</h1>
// //           <p className="subtitle">Track API usage, performance & limits</p>
// //         </div>

// //         {loading && <p className="status">Loading logs...</p>}
// //         {error && <p className="error">{error}</p>}

// //         {/* Requests Per Minute Chart */}
// //         {!loading && requestsPerMinute.length > 0 && (
// //           <div className="chart-section">
// //             <h2>Requests Per Minute</h2>
// //             <ResponsiveContainer width="100%" height={300}>
// //               <LineChart data={requestsPerMinute}>
// //                 <CartesianGrid strokeDasharray="3 3" />
// //                 <XAxis dataKey="time" />
// //                 <YAxis allowDecimals={false} />
// //                 <Tooltip />
// //                 <Line
// //                   type="monotone"
// //                   dataKey="requests"
// //                   stroke="#00f5d4"
// //                   strokeWidth={2}
// //                   dot={{ fill: '#00f5d4', r: 4 }}
// //                 />
// //               </LineChart>
// //             </ResponsiveContainer>
// //           </div>
// //         )}

// //         {/* Logs Table & Summary */}
// //         <div className="logs-section">
// //           <h2>Recent Logs</h2>
// //           <div className="logs-layout">
// //             <div className="logs-table-container">
// //               {logs.length > 0 ? (
// //                 <table className="logs-table">
// //                   <thead>
// //                     <tr>
// //                       <th>Status</th>
// //                       <th>Method</th>
// //                       <th>Endpoint</th>
// //                       <th>IP Address</th>
// //                       <th>API Key ID</th>
// //                       <th>Response Time</th>
// //                       <th>Time</th>
// //                     </tr>
// //                   </thead>
// //                   <tbody>
// //                     {logs.map((log) => (
// //                       <tr key={log.id}>
// //                         <td>
// //                           <span
// //                             className={`status-code ${
// //                               log.status_code >= 400 ? 'error' : 'success'
// //                             }`}
// //                           >
// //                             {log.status_code}
// //                           </span>
// //                           {log.rate_limit_hit && (
// //                             <span className="rate-limit">Rate Limited</span>
// //                           )}
// //                         </td>
// //                         <td className="method">{log.method}</td>
// //                         <td className="endpoint">{log.endpoint}</td>
// //                         <td className="ip-address">{log.ip_address || 'N/A'}</td>
// //                         <td className="apikey-id">{log.apikey_id || 'N/A'}</td>
// //                         <td className="response-time">
// //                           {log.response_time_ms || 0} ms
// //                         </td>
// //                         <td className="timestamp">
// //                           {new Date(log.createdAt).toLocaleString()}
// //                         </td>
// //                       </tr>
// //                     ))}
// //                   </tbody>
// //                 </table>
// //               ) : (
// //                 !loading && <p className="no-data">No logs available yet.</p>
// //               )}
// //             </div>

// //             <div className="logs-summary">
// //               <h3>API Usage Summary</h3>
// //               <div className="stats-grid">
// //                 <div className="stat-card">
// //                   <span className="stat-number">{logs.length}</span>
// //                   <span className="stat-label">Total Requests</span>
// //                 </div>
// //                 <div className="stat-card">
// //                   <span className="stat-number success-count">
// //                     {logs.filter((l) => l.status_code < 400).length}
// //                   </span>
// //                   <span className="stat-label">Successful</span>
// //                 </div>
// //                 <div className="stat-card">
// //                   <span className="stat-number error-count">
// //                     {logs.filter((l) => l.status_code >= 400).length}
// //                   </span>
// //                   <span className="stat-label">Errors</span>
// //                 </div>
// //               </div>
// //             </div>
// //           </div>
// //         </div>
// //       </div>
// //     </div>
// //   );
// // };

// // export default Dashboard;


// import React, { useEffect, useState, useCallback } from 'react';
// import { LineChart, Line, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer } from 'recharts';
// import './Dashboard.css';
// import { apiKeyAPI } from '../../services/apiKeyAPI';
// import { analyticsAPI } from '../../services/analytics';

// const Dashboard = () => {
//   const [logs, setLogs] = useState([]);
//   const [apiKeys, setApiKeys] = useState([]);
//   const [requestsPerMinute, setRequestsPerMinute] = useState([]);
//   const [loading, setLoading] = useState(true);
//   const [error, setError] = useState('');

//   // Process logs for chart - Now with 12-hour format
//   const processRequestsPerMinute = useCallback((logsData) => {
//     const counts = {};
//     logsData.forEach((log) => {
//       if (!log.createdAt) return;
//       const d = new Date(log.createdAt);
      
//       // Create 12-hour time key with AM/PM
//       let hours = d.getHours();
//       const minutes = d.getMinutes();
//       const ampm = hours >= 12 ? 'PM' : 'AM';
//       hours = hours % 12;
//       hours = hours ? hours : 12; // 0 should be 12

//       const key = `${hours}:${minutes.toString().padStart(2, '0')} ${ampm}`;

//       counts[key] = (counts[key] || 0) + 1;
//     });

//     const chartData = Object.keys(counts)
//       .map((time) => ({ time, requests: counts[time] }))
//       .sort((a, b) => {
//         // Sort by actual time (convert back to 24hr for proper sorting)
//         const timeToMinutes = (t) => {
//           const [timePart, period] = t.split(' ');
//           let [h, m] = timePart.split(':').map(Number);
//           if (period === 'PM' && h !== 12) h += 12;
//           if (period === 'AM' && h === 12) h = 0;
//           return h * 60 + m;
//         };
//         return timeToMinutes(a.time) - timeToMinutes(b.time);
//       });

//     setRequestsPerMinute(chartData);
//   }, []);

//   // Fetch API Keys
//   const fetchApiKeys = useCallback(async () => {
//     try {
//       const response = await apiKeyAPI.getAll();
//       const userKeys = response.data.data;
//       setApiKeys(userKeys);
//     } catch (err) {
//       console.error('Failed to fetch API keys:', err);
//     }
//   }, []);

//   // Fetch Logs - Oldest first, Newest last
//   const fetchLogs = useCallback(async () => {
//     try {
//       setLoading(true);
//       setError('');

//       const res = await analyticsAPI.getAnalytics();
//       let logsData = res.data.data || res.data || [];

//       // Sort: Oldest → Newest (Newest at bottom)
//       const sortedLogs = [...logsData].sort((a, b) => 
//         new Date(a.createdAt) - new Date(b.createdAt)
//       );

//       setLogs(sortedLogs);
//       processRequestsPerMinute(sortedLogs);
//     } catch (err) {
//       console.error('Failed to fetch logs:', err);
//       setError('Failed to load analytics logs');
//     } finally {
//       setLoading(false);
//     }
//   }, [processRequestsPerMinute]);

//   // Load data when component mounts
//   useEffect(() => {
//     fetchLogs();
//     fetchApiKeys();
//   }, [fetchLogs, fetchApiKeys]);

//   return (
//     <div className="dashboard-page">
//       <div className="dashboard-container">
//         <div className="dashboard-header">
//           <h1>API Analytics</h1>
//           <p className="subtitle">Track API usage, performance & limits</p>
//         </div>

//         {loading && <p className="status">Loading logs...</p>}
//         {error && <p className="error">{error}</p>}

//         {/* Requests Per Minute Chart */}
//         {!loading && requestsPerMinute.length > 0 && (
//           <div className="chart-section">
//             <h2>Requests Per Minute</h2>
//             <ResponsiveContainer width="100%" height={300}>
//               <LineChart data={requestsPerMinute}>
//                 <CartesianGrid strokeDasharray="3 3" />
//                 <XAxis 
//                   dataKey="time" 
//                   angle={-45}
//                   textAnchor="end"
//                   height={70}
//                 />
//                 <YAxis allowDecimals={false} />
//                 <Tooltip />
//                 <Line
//                   type="monotone"
//                   dataKey="requests"
//                   stroke="#00f5d4"
//                   strokeWidth={2}
//                   dot={{ fill: '#00f5d4', r: 4 }}
//                 />
//               </LineChart>
//             </ResponsiveContainer>
//           </div>
//         )}

//         {/* Logs Table & Summary */}
//         <div className="logs-section">
//           <h2>Recent Logs</h2>
//           <div className="logs-layout">
//             <div className="logs-table-container">
//               {logs.length > 0 ? (
//                 <table className="logs-table">
//                   <thead>
//                     <tr>
//                       <th>Status</th>
//                       <th>Method</th>
//                       <th>Endpoint</th>
//                       <th>IP Address</th>
//                       <th>API Key ID</th>
//                       <th>Response Time</th>
//                       <th>Time</th>
//                     </tr>
//                   </thead>
//                   <tbody>
//                     {logs.map((log) => (
//                       <tr key={log.id}>
//                         <td>
//                           <span
//                             className={`status-code ${
//                               log.status_code >= 400 ? 'error' : 'success'
//                             }`}
//                           >
//                             {log.status_code}
//                           </span>
//                           {log.rate_limit_hit && (
//                             <span className="rate-limit">Rate Limited</span>
//                           )}
//                         </td>
//                         <td className="method">{log.method}</td>
//                         <td className="endpoint">{log.endpoint}</td>
//                         <td className="ip-address">{log.ip_address || 'N/A'}</td>
//                         <td className="apikey-id">{log.apikey_id || 'N/A'}</td>
//                         <td className="response-time">
//                           {log.response_time_ms || 0} ms
//                         </td>
//                         <td className="timestamp">
//                           {new Date(log.createdAt).toLocaleString('en-US', {
//                             year: 'numeric',
//                             month: 'short',
//                             day: 'numeric',
//                             hour: 'numeric',
//                             minute: '2-digit',
//                             second: '2-digit',
//                             hour12: true,
//                           })}
//                         </td>
//                       </tr>
//                     ))}
//                   </tbody>
//                 </table>
//               ) : (
//                 !loading && <p className="no-data">No logs available yet.</p>
//               )}
//             </div>

//             <div className="logs-summary">
//               <h3>API Usage Summary</h3>
//               <div className="stats-grid">
//                 <div className="stat-card">
//                   <span className="stat-number">{logs.length}</span>
//                   <span className="stat-label">Total Requests</span>
//                 </div>
//                 <div className="stat-card">
//                   <span className="stat-number success-count">
//                     {logs.filter((l) => l.status_code < 400).length}
//                   </span>
//                   <span className="stat-label">Successful</span>
//                 </div>
//                 <div className="stat-card">
//                   <span className="stat-number error-count">
//                     {logs.filter((l) => l.status_code >= 400).length}
//                   </span>
//                   <span className="stat-label">Errors</span>
//                 </div>
//               </div>
//             </div>
//           </div>
//         </div>
//       </div>
//     </div>
//   );
// };

// export default Dashboard;


import React, { useEffect, useState, useCallback } from 'react';
import { LineChart, Line, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer } from 'recharts';
import './Dashboard.css';
import { apiKeyAPI } from '../../services/apiKeyAPI';
import { analyticsAPI } from '../../services/analytics';

const Dashboard = () => {
  const [logs, setLogs] = useState([]);
  const [apiKeys, setApiKeys] = useState([]);
  const [requestsPerMinute, setRequestsPerMinute] = useState([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState('');

  // Process logs for chart - Proper chronological time series
  const processRequestsPerMinute = useCallback((logsData) => {
    const counts = {};

    logsData.forEach((log) => {
      if (!log.createdAt) return;
      const d = new Date(log.createdAt);
      
      // Full minute key: "2026-05-17 09:45" (preserves day + time)
      const year = d.getFullYear();
      const month = String(d.getMonth() + 1).padStart(2, '0');
      const day = String(d.getDate()).padStart(2, '0');
      const hour = String(d.getHours()).padStart(2, '0');
      const minute = String(d.getMinutes()).padStart(2, '0');

      const key = `${year}-${month}-${day} ${hour}:${minute}`;
      
      counts[key] = (counts[key] || 0) + 1;
    });

    // Convert to array and sort chronologically (oldest first)
    const chartData = Object.keys(counts)
      .map((timeKey) => ({
        time: timeKey,
        // Display format for chart (you can customize)
        displayTime: new Date(timeKey.replace(' ', 'T')).toLocaleTimeString('en-US', {
          hour: 'numeric',
          minute: '2-digit',
          hour12: true
        }),
        requests: counts[timeKey]
      }))
      .sort((a, b) => new Date(a.time) - new Date(b.time)); // Oldest → Newest

    setRequestsPerMinute(chartData);
  }, []);

  // Fetch API Keys
  const fetchApiKeys = useCallback(async () => {
    try {
      const response = await apiKeyAPI.getAll();
      setApiKeys(response.data.data || []);
    } catch (err) {
      console.error('Failed to fetch API keys:', err);
    }
  }, []);

  // Fetch Logs
  const fetchLogs = useCallback(async () => {
    try {
      setLoading(true);
      setError('');

      const res = await analyticsAPI.getAnalytics();
      let logsData = res.data.data || res.data || [];

      // Sort logs: Oldest → Newest
      const sortedLogs = [...logsData].sort((a, b) => 
        new Date(a.createdAt) - new Date(b.createdAt)
      );

      setLogs(sortedLogs);
      processRequestsPerMinute(sortedLogs);
    } catch (err) {
      console.error('Failed to fetch logs:', err);
      setError('Failed to load analytics logs');
    } finally {
      setLoading(false);
    }
  }, [processRequestsPerMinute]);

  useEffect(() => {
    fetchLogs();
    fetchApiKeys();
  }, [fetchLogs, fetchApiKeys]);

  return (
    <div className="dashboard-page">
      <div className="dashboard-container">
        <div className="dashboard-header">
          <h1>API Analytics</h1>
          <p className="subtitle">Track API usage, performance & limits</p>
        </div>

        {loading && <p className="status">Loading logs...</p>}
        {error && <p className="error">{error}</p>}

        {/* Requests Per Minute Chart - Chronological */}
        {!loading && requestsPerMinute.length > 0 && (
          <div className="chart-section">
            <h2>Requests Per Minute</h2>
            <ResponsiveContainer width="100%" height={320}>
              <LineChart data={requestsPerMinute}>
                <CartesianGrid strokeDasharray="3 3" />
                <XAxis 
                  dataKey="displayTime" 
                  angle={-45}
                  textAnchor="end"
                  height={80}
                  interval="preserveStartEnd" // or 0 for all labels
                />
                <YAxis allowDecimals={false} />
                <Tooltip 
                  labelFormatter={(label, payload) => {
                    // Show full datetime in tooltip
                    const original = payload?.[0]?.payload?.time;
                    return original ? new Date(original).toLocaleString() : label;
                  }}
                />
                <Line
                  type="monotone"
                  dataKey="requests"
                  stroke="#00f5d4"
                  strokeWidth={2.5}
                  dot={{ fill: '#00f5d4', r: 3.5 }}
                  activeDot={{ r: 6 }}
                />
              </LineChart>
            </ResponsiveContainer>
          </div>
        )}

        {/* Logs Table & Summary - unchanged */}
        <div className="logs-section">
          <h2>Recent Logs</h2>
          <div className="logs-layout">
            <div className="logs-table-container">
              {logs.length > 0 ? (
                <table className="logs-table">
                  <thead>
                    <tr>
                      <th>Status</th>
                      <th>Method</th>
                      <th>Endpoint</th>
                      <th>IP Address</th>
                      <th>API Key ID</th>
                      <th>Response Time</th>
                      <th>Time</th>
                    </tr>
                  </thead>
                  <tbody>
                    {logs.map((log) => (
                      <tr key={log.id}>
                        <td>
                          <span className={`status-code ${log.status_code >= 400 ? 'error' : 'success'}`}>
                            {log.status_code}
                          </span>
                          {log.rate_limit_hit && <span className="rate-limit">Rate Limited</span>}
                        </td>
                        <td className="method">{log.method}</td>
                        <td className="endpoint">{log.endpoint}</td>
                        <td className="ip-address">{log.ip_address || 'N/A'}</td>
                        <td className="apikey-id">{log.apikey_id || 'N/A'}</td>
                        <td className="response-time">{log.response_time_ms || 0} ms</td>
                        <td className="timestamp">
                          {new Date(log.createdAt).toLocaleString('en-US', {
                            year: 'numeric',
                            month: 'short',
                            day: 'numeric',
                            hour: 'numeric',
                            minute: '2-digit',
                            second: '2-digit',
                            hour12: true,
                          })}
                        </td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              ) : (
                !loading && <p className="no-data">No logs available yet.</p>
              )}
            </div>

            <div className="logs-summary">
              <h3>API Usage Summary</h3>
              <div className="stats-grid">
                <div className="stat-card">
                  <span className="stat-number">{logs.length}</span>
                  <span className="stat-label">Total Requests</span>
                </div>
                <div className="stat-card">
                  <span className="stat-number success-count">
                    {logs.filter((l) => l.status_code < 400).length}
                  </span>
                  <span className="stat-label">Successful</span>
                </div>
                <div className="stat-card">
                  <span className="stat-number error-count">
                    {logs.filter((l) => l.status_code >= 400).length}
                  </span>
                  <span className="stat-label">Errors</span>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
};

export default Dashboard;